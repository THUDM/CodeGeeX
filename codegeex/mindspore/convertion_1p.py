...
def load_model(args_opt):
    ...
    mox.file.copy(
        os.path.join(args_opt.load_ckpt_path, "rank_0", ckpt_name),
    ...
    if not os.path.exists(
        f'/home/work/sfs/cache/{os.environ["BATCH_JOB_ID"]}/1/rank_{rank}'
    ):
    ...
    for k, weight in pangu_alpha.parameters_dict().items():
        print(k)
        np.save(
            os.path.join("/home/work/sfs/cache/ckpts_npy/", f"{k}.npy"),
            weight.asnumpy(),
        )
