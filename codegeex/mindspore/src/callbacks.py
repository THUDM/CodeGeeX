...
class LossCallBack(Callback):
    ...
    def __init__(
        ...
        print(
            "load has trained epoch :{} and step: {}".format(
                has_trained_epoch, has_trained_step
            ),
            flush=True,
        )
    ...
    def step_end(self, run_context):
        ...
        if self._dataset_size > 0 and self.local_rank % 8 == 0:
            ...
            if self.summary_writer is not None:
                print(
                    f"writing: {loss_value.item()}, {cb_params.net_outputs[2].asnumpy()}"
                )
...
class EvalCallBack(Callback):
    ...
    def __init__(
        ...
        self.strategy_ckpt_save_file = context.get_auto_parallel_context(
            "strategy_ckpt_save_file"
        )
        self.strategy_ckpt_load_file = context.get_auto_parallel_context(
    ...
    def step_end(self, run_context):
        ...
        if self.summary_writer is not None:
            ...
            tag = (
                ...
                else f"validaton_loss/{self.lang}"
            )
            self.summary_writer.add_scalar(
                tag=tag,
        ...
        context.set_auto_parallel_context(
            strategy_ckpt_save_file=self.strategy_ckpt_save_file,
...
class SaveCheckpointCallback(Callback):
    ...
    def syn_files(self):
        process = Process(
            target=mox.file.copy_parallel,
            args=(self.cache_dir, self.bucket),
