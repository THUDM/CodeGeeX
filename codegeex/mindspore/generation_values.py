...
def load_model(args_opt):
    ...
    if args_opt.distribute == "false":
        ...
        predict_layout = model_predict.infer_predict_layout(
            inputs_np, init_true, batch_valid_length
        )
        ...
        _ = model_predict.infer_predict_layout(
            inputs_np_1, init_false, batch_valid_length
        )
    else:
...
def run_predict(model_predict, config, args_opt, rank):
    ...
    input_ids = np.concatenate(
        (input_ids, np.ones(2048 - valid_length, dtype=np.int32) * 50256)
    )
    ...
    output_logits = model_predict.predict(
        Tensor(input_ids, mstype.int32), init, batch_valid_length
    )
    output = output_logits.asnumpy()
    if rank == 0:
        np.save(
            "/home/work/sfs/xx/pangu_alpha_code/output_6_7375_8.13.npy", output
        )  # TODO: set as current save path
        os.system(
            "chmod 777 /home/work/sfs/xx/pangu_alpha_code/output_6_7375_8.13.npy"
        )  # TODO: set as current save path
